...
sub _logparse {
	...
	#my $time = timelocal($7,$6,$5,$4,$3-1,$2); https://stackoverflow.com/questions/7726514/how-to-convert-text-date-to-timestamp
	    chomp $_;        
	    $_=~m/^((\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2}):(\d{2})[+:\d]+)[\s]+(input|output),\s+(\w+)/;
...
sub _statvalidity {
    ...
    $self->{lostdate} = join " :non-digit|intervalost> ", map {POSIX::strftime("%Y-%m-%d %H:%M:%S",localtime($_))} @lost;
        ...
        if (exists $valid_data->{$old}){
         ...
         if ($valid_data->{$old} > $valid_data->{$_} ){  # here we pass by
          $old=$_; 
             ...
             $self->{lostdate} .= " :counter drop> ". ${\POSIX::strftime("%Y-%m-%d %H:%M:%S",localtime($_))};
             next;
         };
...
my $fileout=shift ||"logfile_calc_uplinks.log";
$fileout=$dir_out.'/'.$fileout;
my $ini='';
