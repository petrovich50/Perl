...
Input args:
["number _month"] , ['2017-09-18 01:50:01', '2017-09-18 01:50:01'],[percent],[_logfile],[delta_time], [datatype], [timeprecision]
date
2017-09-18 01:50:01
...
sub initial {
    ...
    $self->{_deltatime}=$_[5] || "600"; # in sec defalt 600
    $self->{_datatype}=$_[6] || "output"; # defalt output
    $self->{_timeprecision}=$_[7] || 0; # 2-5 in sec, defalt 1 - exact precision
    $self->{_lostdate}="";
    # print "\n SELF  ",Dumper $self,"\n";
...
sub _statvalidity {
    ...
    @lost = @{[grep {($reshash{$_}->[0] =~/\D+/) and ($_ >= $time_beg) and ($_ <= $time_end)} sort {$a <=> $b} keys %reshash]}; #none digite data
...
sub _bandwidth_rate {
    ...
    # print "\n do _bandwidth_rate number \n",scalar @{[sort {$a <=> $b} keys $valid_data]}, "\n __endinp\n";
     if (ref($valid_data) eq "HASH" ){
      foreach (@{[sort {$a <=> $b} keys $valid_data]}){  # body of approximation / We use here simple direct approximation delta value/ delta time min
        if (exists $valid_data->{$old}){
         # $value_old=0 if ($valid_data->{$old} > $valid_data->{$_});
         if ($valid_data->{$old} > $valid_data->{$_} ){  #alternative code
         $old=$_; 
...
sub burst {
    ...
    # print "\n 2222222 ",$self->{period_end};
        ...
        if (ref($rate) eq "HASH" ) {
          ...
          # print Dumper "\n burst list here \n",\@burst_rate,"\n";
          my $countall=$#burst_rate;
          # print "\n befor burst list here \n","\n", scalar @burst_rate, "\n";
          while (($#burst_rate /$countall) > (1-$self->{_percentile})){ shift @burst_rate;};
          # print "\n burst list here \n","\n ", scalar @burst_rate, " \n";
          return (8/1000000000) * shift @burst_rate;  #octet/Gbite factoring here
        }else{
...
my $fileout=shift || "/tmp/burst_calc.log";
$ini = $obj->initial(shift || "current", shift || "", shift || "") ; #,[percent],[_logfile],[delta_time], [datatype], [timeprecision])
my $bur=$obj->burst();
open (my $fh,">>", $fileout);
if ($ini!~/^no\s+/){
  print $fh "Burst: ".$bur;
